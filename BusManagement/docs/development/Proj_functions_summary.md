...
| `Route` | Tuyến đường | `distanceKm`, `estimatedDuration`, `suitableBusType` (N-1 `BusType`), `routeStations` (1-N, `@BatchSize(20)`). **Không còn** hai cột text `departurePoint`/`destinationPoint` (đã xoá — `Route.java` chỉ còn comment "ĐÃ XÓA"; đính chính 2026-09-19, câu cũ ở đây mâu thuẫn với `database_schema.md`): điểm đi/đến được **suy ra** từ `routeStations` theo `stopOrder` qua `getDepartureStation()`/`getDestinationStation()`, và `getDeparturePointDisplay()`/`getDestinationPointDisplay()` trả tên trạm (hoặc "(Chưa gán trạm)") cho tầng view; method `getSuitableBusType()` chỉ trả field, không có AI logic dù docstring nói có |
...
- `createManualTrip()` dùng `changeStatusToActive()` trực tiếp (không qua `updateTripStatus()`) vì đây là trường hợp **tạo**: `Trip` còn transient (id `null`, chưa có bản ghi nào trong DB) ⇒ thật sự không có "from state" nào để kiểm. Lưu ý trạng thái của nó lúc đó **không** phải mặc định `PENDING_APPROVAL` của entity — `AdminTripManagementController.createTrip():117` đã set thẳng `ACTIVE` trước khi gọi service. Hệ quả: **không được gom guard của #20 vào trong `changeStatusToActive()`**, vì nó sẽ ném lỗi ở đúng lối tạo chuyến này.
- `approveTrip()`/`confirmAutoAssignedTrip()` **cũng** dùng `changeStatusToActive()` trực tiếp, nhưng **KHÔNG** vì lý do trên: cả hai mở đầu bằng `tripRepository.findById(tripId)`, tức thao tác trên bản ghi đã tồn tại và **có** from-state. Từ bản sửa lỗi **#20**, cả hai gọi `requirePendingApproval(trip)` ngay đầu method, nên chỉ chuyến `PENDING_APPROVAL` mới được duyệt — đúng dòng duy nhất mà whitelist FSM cho phép đi tới `ACTIVE`.
  > ⚠️ **Đính chính (2026-08-12).** Trước đó dòng này gộp cả ba method vào một câu: *"đây là trường hợp tạo/duyệt — không có 'from state' cần kiểm tra transition"*. Câu đó **đúng với `createManualTrip()` và sai với hai method kia** — một tính chất của method thứ nhất bị khái quát cho cả ba. Chính câu sai đó là thứ đã khiến lỗ #20 sống sót qua **ba** lần rà toàn dự án: đợt audit "không bypass FSM" ghi ở gạch đầu dòng trên đã dừng lại ở đây vì tài liệu nói không có gì phải kiểm. Cùng khuôn với lỗi #14, nơi một câu javadoc sai cũng đã che lỗi khỏi các lần rà trước.
...
- **Guard thứ hai trong `updateBus()` — không cho HẠ loại xe (Group C(d), 2026-09-23):** chặn khi sức chứa của loại mới **nhỏ hơn số ghế lớn nhất** mà các chuyến chưa kết thúc của xe đang mở bán (`TripRepository.findMaxTotalSeatsForBus`, cùng tập trạng thái với guard `REPAIRING` — nay gom thành hằng số `UNFINISHED_TRIP_STATUSES` để hai guard không giữ hai bản sao). Đây là cửa sau của luật #22: luật đó nằm ở `validateBusForTrip()` nên chỉ kiểm lúc **gán xe vào chuyến**, không kiểm lúc đổi loại xe — hạ xe Ghế ngồi (50) xuống Limousine (22) làm mọi chuyến đang mở bán quá số chỗ kèm báo THÀNH CÔNG. Luật phát biểu là **"không làm tệ hơn"**: chỉ chặn khi loại mới nhỏ hơn loại đang có, nên giữ nguyên hoặc nâng lên luôn được phép (nâng lên chính là đường Admin tự khắc phục một vi phạm tồn đọng). Bất đối xứng đó là cố ý — xe 20 đã vi phạm #22 sẵn từ trước (chuyến 6, `DEPARTED`, 40 ghế trên xe 22 chỗ, giữ làm lịch sử), nếu guard chỉ hỏi "sức chứa mới < ghế đang bán" thì xe đó không sửa nổi cả odometer. Xe chưa gán loại thì bỏ qua, đúng cách #22 bỏ qua.
...
- **`DataInitializer` CÓ seed `RouteStation`:** `createRoute()` (`DataInitializer:237`) gọi `linkRouteStation()` hai lần cho mỗi tuyến (`:244-245`, stopOrder 1 và 2), và cả 6 tuyến seed đều đi qua hàm đó. Lối ghi `routes` còn lại là `RouteService.createRoute()`/`updateRoute()`, nơi `validateRoute()` đòi tối thiểu 2 trạm — nên **không có đường nào tạo ra một tuyến không có trạm**, và điểm đi/đến luôn suy được từ `routeStations`.
  *(Câu cũ ở đây nói `DataInitializer` "(tùy version) **không seed `RouteStation`**", "`route_stations` có thể trống" và "route vẫn dùng String tự do cho điểm đi/đến". Cả ba vế đều sai, và **sai từ trước khi file này vào repo**: seeding `RouteStation` cùng việc xóa hai field String được làm ở `a3b0e63` **2026-06-27**, còn file này vào repo ở `07af949` **2026-07-14** — muộn hơn 17 ngày. Cùng hình dạng với đính chính `project_report.md` 🟣 #3 ngày 2026-09-19 ("đã cũ nửa câu ngay từ lúc được đưa vào `docs/`"). Đã sửa 2026-09-22.)*
...
- `GET /available-resources?departure=...&arrival=...` (ISO LocalDateTime): trả JSON `{buses: [...], drivers: [...], assistants: [...]}` gồm các xe/tài xế/phụ xe **thực sự rảnh** trong khung giờ, dùng cho JS phía form tạo trip lọc động (khắc phục một phần vấn đề "hiển thị toàn bộ DB" từng được ghi nhận ở review trước). Validate `arrival` phải sau `departure`.
- **`drivers` và `assistants` là HAI danh sách riêng (lỗi #13, sửa 2026-09-11).** `assistants` luôn là **siêu tập** của `drivers`: nó thiếu đúng bộ lọc trần 8h giờ lái, vì `validateStaffForTrip()` không áp trần đó cho vai trò phụ xe. Template phải đổ `drivers` vào dropdown Tài xế chính **và** các slot Tài xế phụ (hai vai trò này cầm lái nên *có* chịu trần), còn `assistants` chỉ vào dropdown Phụ xe. Dùng một danh sách cho cả ba chính là lỗi #13.
- DTO trả về cho bus: `id, licensePlate, typeName, capacity, brand`. DTO cho driver/assistant: `userId, fullName, licenseNumber, experienceYears` (cùng một `toDriverDto`).
- Nội bộ gọi `tripService.getAvailableBusesForTimeRange()`, `getAvailableDriversForTimeRange()` và `getAvailableAssistantsForTimeRange()` (xem 6.6).
...
|---|---|---|
| `getAvailableBusesForTrip(tripId)` | Lấy xe rảnh cho 1 trip **đã tồn tại** (dùng departure/arrival của chính trip đó) | Cần `tripId` có trước, loại trừ chính trip này khi check bận. **Mời mọi loại xe**, xe đúng `route.suitableBusType` xếp đầu rồi mới theo km (Group C(a), 2026-09-23 — loại xe là gợi ý; trước đó lọc cứng theo loại) |
...
| `getAvailableDriversForTimeRange(departure, arrival)` | Tương tự cho driver | Dùng cho REST API `/api/admin/trips/available-resources`; CHỈ cho vai trò cầm lái (tài xế chính + tài xế phụ) |
| `getAvailableAssistantsForTrip(tripId)` | Lấy **phụ xe** khả dụng cho 1 trip đã tồn tại | Màn Phê Duyệt. **Không** áp trần 8h |
| `getAvailableAssistantsForTimeRange(departure, arrival)` | Lấy **phụ xe** khả dụng theo khung giờ tự do | Form TẠO chuyến qua REST API. **Không** áp trần 8h (lỗi #13) |

Bốn hàm cho vai trò **cầm lái** áp cùng bộ điều kiện cốt lõi: `isActive`, `isLicenseValid(ngày khởi hành)`, giờ lái trong ngày + effective hours ≤ 8h, không bận trong cửa sổ nghỉ tối thiểu.

Hai hàm `...Assistants...` áp **ba** điều kiện đầu trừ trần 8h — đúng bằng những gì `validateStaffForTrip()` kiểm cho vai trò phụ xe, vì phụ xe không cầm lái (quy ước "phụ xe = 0h" trong `sumDrivingHours()`). Hệ quả: tập phụ xe luôn là **siêu tập** của tập tài xế với cùng tham số, nên quy tắc một chiều của §8 (*dropdown không được mời người validator sẽ từ chối*) vẫn được giữ. Hai hàm này khác nhau ở hai điểm thuần cơ học, ghi rõ trong javadoc của hàm thứ hai: `excludeTripId` (`tripId` vs `null`) và cách nạp (`findAll()` vs `findAllWithUser()`, tránh N+1 khi dựng JSON).
...
> **Đã đối soát TỪNG mục của §12 với code ngày 2026-09-22** (grep + đọc source, không đọc docs — `git log -S` để định ngày từng claim). Kết quả: **4 mục sai** (1, 3, 6, 9), **1 mục là câu hỏi chưa ai trả lời** (5), **1 mục đúng nửa** (11); **4 mục đúng và vẫn mở** (2, 4, 7, 8); mục 10 đã gạch từ trước. Mỗi mục nay tự khai trạng thái, nên danh sách này **không phải backlog** — chỉ mục 2/4/7/8 là việc còn nợ.
>
> **Nguyên nhân gốc, ghi ra để không tái diễn:** tiêu đề §12 nói *"tổng hợp từ code"*, nhưng danh sách được mang nguyên từ một bản phân tích CŨ vào repo mà không đối soát lại với code lúc đó. Bằng chứng theo ngày: mục 3 được sửa ở `a06b73a` **2026-07-14 08:30**, mục 6 ở `a3b0e63` **2026-06-27**, còn file này vào repo ở `07af949` **2026-07-14 09:52** — tức mục 3 đã sai **1 giờ 21 phút** trước khi được commit, mục 6 sai trước **17 ngày**. Mục 1 thì chưa bao giờ đúng. Đây đúng khuôn mà THESIS_ROADMAP §9 gọi là loại lỗi đắt nhất của project ("một câu sai trong tài liệu"), và cùng hình dạng với đính chính `project_report.md` 🟣 #3. **Bài học:** một danh sách "điểm chưa nhất quán" phải được đối soát lại với code **tại thời điểm commit nó**, không phải tại thời điểm viết nó.

1. ~~**2 luồng tạo Bus song song:** `AdminBusController`→`BusService` (có validate) vs `AdminController`→`AdminService` (không validate gì).~~ — **SAI, và chưa bao giờ đúng.** Đã kiểm 2026-09-22: `AdminController` chỉ có 3 mapping (`GET /dashboard`, `GET /users/new`, `POST /users/save`) và `AdminService` chỉ có `createNewUser()` + `requireUsernameAvailable()` — không một dòng nào về `Bus`. Mọi lối ghi `buses`: `BusService:73/147/210` (giao diện), `DataInitializer:311` + `HistoricalDataBackfill:274` (seeder), `TripService:660/672` (FSM đồng bộ `BusStatus`). `AdminController` **có** inject `BusRepository` nhưng chỉ để `count()` cho thẻ dashboard (`:35`) — đó là Warn #4 (controller inject repository), **không phải** một luồng tạo xe thứ hai.
   *(§3 của chính file này đã đính chính đúng câu đó ngày 2026-08-05 khi rà lối ghi cho bản sửa #16 — nhưng bản sao thứ hai ở đây bị bỏ sót, nên hai mục của cùng một file nói ngược nhau suốt 48 ngày. Đã sửa 2026-09-22.)*
2. **Tạo trip thủ công luôn ACTIVE ngay**, bỏ qua `PENDING_APPROVAL`, không nhất quán với flow AI-suggested trip. — **ĐÚNG, CÒN MỞ** (kiểm 2026-09-22: `AdminTripManagementController:161` set `TripStatus.ACTIVE` trước khi gọi `createManualTrip()`). Đây là Warn #2 trong `project_report.md`, đang chờ một quyết định của chủ dự án.
3. ~~**Ràng buộc giờ lái áp dụng sai cho vai trò Assistant** trong `findBestAvailableDriver()` (AI auto-assign) — phụ xe không lái nhưng vẫn bị tính giờ lái như tài xế khi đang được xét chọn.~~ — **ĐÃ SỬA ở `a06b73a` (2026-07-14), trước khi mục này vào repo.** `findBestAvailableDriver()` nhận `boolean isAssistantRole` (`TripService:443/455`) và bộ lọc trần 8h bị bỏ qua cho vai trò phụ xe: `.filter(d -> isAssistantRole || getDrivingHoursForDate(...) + effectiveHours <= 8.0)` (`:478`); `autoAssignResources()` truyền `true` đúng một lần, cho phụ xe (`:271-272`). Luật này khớp `validateStaffForTrip()` — nơi cũng không áp trần giờ cho phụ xe — nên toàn hệ thống chỉ có một định nghĩa.
   *(`project_report.md` 🟣 #1 đã ghi "✅ ĐÃ FIX" cho đúng bản sửa này. Hai register nói ngược nhau từ ngày đầu; đã đồng bộ 2026-09-22.)*
4. ~~**`Route.getSuitableBusType()`** docstring nói có "AI logic" theo khoảng cách nhưng thực tế chỉ trả field, luôn trả `null` nếu Admin chưa gán tay.~~ — **ĐÃ SỬA 2026-09-23 cùng Group C(a):** lời hứa đã xoá, javadoc nay nói đúng — trả loại xe **gợi ý** Admin chọn, `null` = không quy định; thân hàm rút về `return this.suitableBusType`. Không cài luật gợi ý theo quãng đường. *(Trước đó:)* **ĐÚNG, CÒN MỞ** (kiểm 2026-09-22: `Route.java:53-66` — javadoc ghi *"hệ thống tự gợi ý dựa trên quãng đường"*, thân hàm `return null`). Chính là loại "câu tài liệu hứa một hành vi code không có" mà THESIS_ROADMAP §9 xếp là lỗi đắt nhất của project; ở đây nó nằm trong javadoc chứ không trong docs. Sửa là **xóa lời hứa trong javadoc**, không phải cài thêm luật gợi ý loại xe (luật đó chưa ai duyệt — xem mục ruling "loại xe ↔ `suitableBusType`" trong `current_bugs_found.md`).
5. ~~**`BusType` từng có field `suitableBusType` trùng lặp với `Route`** (… cần verify lại trong source thật).~~ — **ĐÃ VERIFY 2026-09-22: đã dọn sạch.** `BusType` chỉ còn 3 field: `id`, `typeName`, `capacity` (`BusType.java:18/21/24`); `suitableBusType` chỉ tồn tại ở `Route.java:51` (đúng chỗ của nó — loại xe phù hợp là thuộc tính của TUYẾN, không phải của loại xe). Mục này vốn là một câu hỏi tự đặt ra cho lần rà sau; nay đã có câu trả lời nên không còn là điểm cần lưu ý.
6. ~~**Route dùng String tự do cho điểm đi/đến**, không liên kết `Station` entity thật — dữ liệu seed 2 bên tên không khớp nhau.~~ — **ĐÃ SỬA ở `a3b0e63` (2026-06-27), trước khi mục này vào repo 17 ngày.** Hai field String đã bị xóa (`Route.java:31-32` chỉ còn comment `// ĐÃ XÓA:`); nguồn sự thật duy nhất là `RouteStation` sắp theo `stopOrder`, điểm đi/đến suy ra qua `getDepartureStation()`/`getDestinationStation()` và hiển thị qua `getDeparturePointDisplay()`/`getDestinationPointDisplay()`. Grep toàn bộ `src/`: **0** tham chiếu tới field cũ; 6 template dùng đúng hai getter hiển thị. Vế "dữ liệu seed 2 bên tên không khớp" cũng hết hiệu lực — cùng commit đó cho `DataInitializer` seed `RouteStation` (xem §4).
   *(`project_report.md` Incon #1 đã ghi "✅ ĐÃ FIX" cho đúng bản sửa này. Đã đồng bộ 2026-09-22.)*
7. **`countBusyTripsAnyRole`/`findAllTripsByDriverOnDate`** là dead code (định nghĩa nhưng không gọi), trùng chức năng với cặp query đang dùng thật. — **ĐÚNG, CÒN MỞ** (kiểm 2026-09-22: grep toàn `src/` — hai tên này chỉ xuất hiện tại chính chỗ khai báo, `TripRepository:207` và `:222`, không một call site nào). Cặp đang dùng thật là `existsOverlappingTripForDriver`/`findTripsForDriverOnDate`. Đây là Incon #3 trong `project_report.md`, vẫn mở.
8. **Password không được mã hóa** khi tạo User (`AdminService.createNewUser` có TODO comment nhưng chưa implement BCrypt). — **ĐÚNG, CÒN MỞ** (kiểm 2026-09-22: `AdminService:51` là dòng comment `// user.setPassword(passwordEncoder.encode(...));`, `DriverService:118` ghi thẳng `rawPassword`, `DataInitializer:284` seed `"123456"`). Không có bean `PasswordEncoder` nào trong project. Gắn với mục 9: chưa có đăng nhập thì mật khẩu chưa được dùng để xác thực ở đâu, nên đây là nợ đi kèm việc bật login — không phải lỗ hổng đang bị khai thác.
9. **Không có phân quyền thực tế** dù có enum `Role` với 3 cấp. Kiểm 2026-09-22, grep toàn `src/main`: **0** `@PreAuthorize`/`hasRole`/`hasAuthority`, **0** `SecurityContextHolder`, **0** bean `PasswordEncoder`, và **không có `UserDetailsService` của riêng project** (tên đó chỉ xuất hiện đúng một lần, ở vế `exclude` loại bỏ user in-memory mặc định của Boot — `BusManagementApplication:24`). `Role` hiện chỉ dùng để **phân loại bản ghi**, không để chặn request: `AdminService:44` từ chối `ROLE_DRIVER`, `DriverService:77` ép `ROLE_DRIVER`, `AdminController:48-49` ẩn `ROLE_DRIVER` khỏi select. **CÒN MỞ, và là Non-Goal có chủ đích** (THESIS_ROADMAP §4: bật Spring Security thật là task riêng cần chủ dự án duyệt; muốn bật thì phải tự cấp một bean `UserDetailsService`).
   *(Câu cũ ở đây viết **"Spring Security tắt hoàn toàn"** — **SAI**. Spring Security nằm trên classpath và auto-config của nó đang **BẬT**; trạng thái permit-all đến từ `SecurityConfig` (`anyRequest().permitAll()`, tắt CSRF/form-login/http-basic), không phải từ việc tắt security. Đây là bản sao thứ tư của đúng câu sai mà Group B(b) đã sửa ngày 2026-09-21 ở `setup_guide.md` §5/§7, `02_project_context.md` §3 và cả hai `application.properties` — file này bị bỏ sót khỏi lần quét đó. Xem `setup_guide.md` §5 là nguồn sự thật duy nhất cho trạng thái bảo mật. Đã sửa 2026-09-22.)*
...
11. **Khoảng cách lớn giữa spec (`03_functional_spec.md`) và code thực tế** — Ticket/Payment/Seat/QR-checkin/Review/Notification/JWT/Driver-app/Client-app đều chưa được lập trình. **Vế đầu ĐÚNG; nhưng phần lớn danh sách đó nay là Non-Goal có chủ đích, không phải việc còn nợ** (THESIS_ROADMAP §4 liệt kê tường minh: không cổng tài xế, không module giá vé, không hủy vé / đánh giá, không notification, không cron job, không bật login thật). Phần Client (tìm chuyến → đặt vé đơn giản → lịch sử) là **Phase 9**, phase cuối, chưa bắt đầu.
    *(Vế cuối của câu cũ — *"chỉ phần Admin Smart Scheduling được triển khai đầy đủ"* — đã **SAI** từ Phase 1: ngoài Smart Scheduling còn có CRUD Tài xế, CRUD Tuyến, Bảng Điều Hành (Phase 1), Quản Lý Sự Cố (Phase 2), API dry-run (Phase 3), bộ sinh dữ liệu lịch sử (Phase 5) và **5 màn Decision Support** (Phase 4/6/7/8). §13 của chính file này đã liệt kê đủ từ 2026-07-22 mà mục này không được cập nhật theo. Đã sửa 2026-09-22.)*
 ...
 *(2026-09-23, Group C(c): `updateRoute()` nay trả về một câu **cảnh báo** — không chặn — khi `distanceKm` đổi trong lúc tuyến còn chuyến `DEPARTED`, vì FSM đọc km của tuyến lúc `COMPLETED` nên km mới sẽ vào odometer của các chuyến đó; controller đưa câu này ra flash `warning` trên danh sách tuyến. Đếm bằng `TripRepository.countByRouteIdAndStatus`.)*
...
- **Quản lý Sự cố** (Phase 2) — entity `Incident` + enum `IncidentType` (5 giá trị: `VEHICLE_BREAKDOWN`, `ACCIDENT`, `ROAD_ISSUE`, `STAFF_ISSUE`, `OTHER`) và `IncidentStatus` (`OPEN`/`IN_PROGRESS`/`RESOLVED`), `IncidentService`, `AdminIncidentController`. `bus` **bắt buộc** (mọi sự cố đều quy về một xe cụ thể), `trip` và `driver` tùy chọn (xe hỏng trong bãi thì không có chuyến; không có đăng nhập nên không suy ra được người báo). Xe của sự cố **không bắt buộc** trùng xe của chuyến (ruling 2026-09-21 — sau khi đổi xe giữa đường, `trip.bus` không sửa được nữa nên sự cố trên xe thay thế chỉ ghi đúng được nhờ trường `bus` độc lập). Dropdown "chuyến xe liên quan" là **phạm vi mời, không phải luật**: `TripService.getRunningAndRecentTrips(since)` — `ACTIVE`/`DEPARTED` mọi ngày + `COMPLETED`/`CANCELLED` khởi hành trong 7 ngày gần đây (`AdminIncidentController.RECENT_TRIP_DAYS`), mới nhất lên đầu; form sửa luôn thêm lại chuyến đang gắn dù đã ra khỏi cửa sổ. Thay cho `getAllTrips()` (2026-09-21) — đổ cả 2.222 chuyến vào một select là 353 KB / 1,2 s mỗi lần mở form và tăng theo mỗi lần backfill. **Không có FSM** cho `IncidentStatus` — Admin đi lại tự do, kể cả mở lại sự cố đã đóng nhầm; quy tắc duy nhất là `resolvedAt` tự đóng dấu khi vào `RESOLVED` và bị xóa khi rời khỏi. Ghi sự cố **không** tự đổi `Bus.status`; chuyển xe sang `REPAIRING` vẫn là thao tác tay có chủ đích. Xe hoặc tài xế còn bản ghi sự cố thì **không xóa cứng được** — kể cả khi sự cố đã `RESOLVED`, vì ràng buộc này bảo vệ toàn vẹn tham chiếu chứ không phải công việc đang mở.
